...
resource "aws_instance" "my_ec2_instance2" {
  ...
  key_name               = "HRUDAY-KEYPAIR" # paste your key-name here, do not use extension '.pem'
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      private_key = file("DevopsProject1/terraform_files/HRUDAY-KEYPAIR.pem") # replace with your key-name 
      user        = "ec2-user"
      host        = self.public_ip
    }
